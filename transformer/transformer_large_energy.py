...
MODEL = "TRANSFORMER_LARGE_ENERGY"
...
PREVIOUS_RESULTS_FILE = os.path.join(RESULTS_DIR, "TRANSFORMER_LARGE_results_20260504_065856.csv")
hp_df = pd.read_csv(PREVIOUS_RESULTS_FILE)

# Grab the hyperparameters from the first row
hp_row = hp_df.iloc[0]
head_size = int(hp_row["head_size"])
num_heads = int(hp_row["num_heads"])
ff_dim = int(hp_row["ff_dim"])
num_blocks = int(hp_row["num_transformer_blocks"])
mlp_units = int(hp_row["mlp_units"])
