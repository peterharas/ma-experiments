...
df_transformer = pd.read_csv('results/TRANSFORMER_results_20260430_193701.csv')

# 2. Extract only necessary columns to avoid clutter
df_tft_sub = df_tft[['spring_id', 'horizon', 'nse']]
df_transformer_sub = df_transformer[['spring_id', 'horizon', 'nse']]
...
for h in horizons:
    ...
    nse_tft = df_h['nse_tft']
    nse_transformer = df_h['nse_transformer']
    
    # Calculate the differences between pairs
    differences = nse_tft - nse_transformer
    ...
    if shapiro_p > 0.05:
        ...
        t_stat, t_p = stats.ttest_rel(nse_tft, nse_transformer)
        ...
        w_stat, w_p = stats.wilcoxon(nse_tft, nse_transformer)
    ...
    if final_p < 0.05:
        print("\nConclusion: There is a STATISTICALLY SIGNIFICANT difference between TFT and Transformer.")
    else:
