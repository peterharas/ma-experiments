...
df_baseline = pd.read_csv('results/BASELINE_LASTKNOWN_results_20260427_080725.csv')

# 2. Extract only necessary columns to avoid clutter
df_xlstm_sub = df_xlstm[['spring_id', 'horizon', 'nse']]
df_baseline_sub = df_baseline[['spring_id', 'horizon', 'nse']]
...
for h in horizons:
    ...
    nse_baseline = df_h['nse_baseline']
    
    # Calculate the differences between pairs
    differences = nse_xlstm - nse_baseline
    ...
    if shapiro_p > 0.05:
        ...
        t_stat, t_p = stats.ttest_rel(nse_xlstm, nse_baseline)
        ...
        w_stat, w_p = stats.wilcoxon(nse_xlstm, nse_baseline)
    ...
    if final_p < 0.05:
        print("\nConclusion: There is a STATISTICALLY SIGNIFICANT difference between xLSTM and the baseline.")
    else:
