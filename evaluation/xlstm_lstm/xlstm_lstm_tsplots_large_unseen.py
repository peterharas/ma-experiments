...
from util.paths import * # Custom colors
my_colors = {
...
# ---------------------------------------------------------
# NEW: Read the unseen spring IDs and filter the dataframe
# ---------------------------------------------------------
# Read the file and convert the IDs to integers
with open(SPRING_LIST_FILE_UNSEEN, 'r') as f:
    unseen_spring_ids = [int(line.strip()) for line in f if line.strip()]

# Filter the merged dataframe to only include these specific spring_ids
df_merged = df_merged[df_merged['spring_id'].isin(unseen_spring_ids)]
# ---------------------------------------------------------
...
for ax, h in zip(axes, horizons):
    ...
    # Formatting for each individual subplot (Updated title to indicate "Unseen")
    ax.set_ylabel('NSE')
    ax.set_title(f'NSE Comparison: LSTM vs xLSTM generalised model (Horizon {h}) - Unseen Springs')
...
# Updated save path so it doesn't overwrite your original plot
plt.savefig(os.path.join('evaluation', 'xlstm_lstm', 'xlstm_lstm_linecharts_large_unseen.png'), dpi=300, bbox_inches='tight')
plt.show()