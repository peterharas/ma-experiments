...
for spring_id in spring_ids:
    ...
    # 4. Freeze specific layers (input projection & backbone, except sLSTM blocks)
    ...
    # Specifically unfreeze any module that is an mLSTMBlock
    for name, module in model.backbone.named_modules():
        if "sLSTMBlock" in module.__class__.__name__:
            for param in module.parameters():
                param.requires_grad = True
    
    # Verify what is unfrozen (Dense, Output, and mLSTMBlocks)
    unfrozen_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    frozen_params = sum(p.numel() for p in model.parameters() if not p.requires_grad)
    print(f"Frozen parameters: {frozen_params:,} | Trainable parameters (Dense, Output, & mLSTM): {unfrozen_params:,}")
    ...
    for i, d in enumerate(FORECAST_DAYS):
        ...
        results.append({
            ...
            "architecture": LM_ARCHITECTURE,
            "frozen_layers": "input_proj, backbone (except sLSTM)"
        })
