...
for spring_id in spring_ids:
    print(f"\n{'='*50}")
    print(f"Running {MODEL} full fine-tuning for {spring_id}...")
    print(f"{'='*50}")
    ...
    # Verify what is unfrozen (entire model)
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Trainable parameters (Entire Model): {trainable_params:,}")

    # 4. Define optimizer to update ALL parameters
    criterion = nn.L1Loss()
    ...
    print(f"Training (Full Fine-tuning) {spring_id}...")
    ...
    for i, d in enumerate(FORECAST_DAYS):
        ...
        plt.title(f"{MODEL} (Full Fine-Tuned) {spring_id} – {d}-Day Ahead Forecast")
        plt.xlabel("Time")
